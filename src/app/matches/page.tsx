...
export default function MatchesPage() {
  ...
  const handleJoin = async (match: any) => {
    ...
    if (user.balance < match.stake) {
      setActionError(`${t.matches_insufficient_balance} (${user.balance} DH / ${match.stake} DH)`);
      setShowRechargeModal(true);
      return;
    }

    const confirmMsg = `${t.matches_join_btn} (${match.stake} DH)؟`;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        >
          ...
          <span>{t.matches_create_btn}</span>
        </button>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 p-4 rounded-2xl bg-zinc-900/60 border border-zinc-800">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 md:pb-0">
          {[
            { id: 'OPEN', label: t.matches_tab_open },
            { id: 'PLAYING', label: t.matches_tab_playing },
            { id: 'DISPUTE', label: t.matches_tab_dispute },
            { id: 'COMPLETED', label: t.matches_tab_completed },
            { id: 'ALL', label: t.matches_tab_all }
          ].map(tab => (
            <button
              key={tab.id}
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-zinc-500 font-semibold uppercase">{t.matches_platform_label}</span>
          <select
          ...
          >
            <option value="ALL">{t.matches_platform_all}</option>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 rounded-3xl bg-zinc-900/30 border border-dashed border-zinc-800 space-y-3">
          ...
          <h3 className="text-base font-bold text-zinc-300">{t.matches_empty_title}</h3>
          <p className="text-xs text-zinc-500">{t.matches_empty_desc}</p>
          <button
                ...
                <div>
                    ...
                    <div>
                      ...
                      {m.status === 'PLAYING' && (
                        <span className="inline-flex items-center gap-1 text-[10px] font-bold text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded-full border border-cyan-500/20">
                          <Clock className="w-3 h-3" /> {t.matches_tab_playing}
                        </span>
                      )}
                      ...
                      {m.status === 'DISPUTE' && (
                        <span className="inline-flex items-center gap-1 text-[10px] font-bold text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded-full border border-rose-500/20">
                          <AlertTriangle className="w-3 h-3" /> {t.matches_tab_dispute}
                        </span>
                      )}
                      {m.status === 'COMPLETED' && (
                        <span className="inline-flex items-center gap-1 text-[10px] font-bold text-zinc-400 bg-zinc-800 px-2 py-0.5 rounded-full">
                          <CheckCircle2 className="w-3 h-3 text-emerald-400" /> {t.matches_tab_completed}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="py-4 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs font-bold text-zinc-500">{t.room_host}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <span className="text-xs font-bold text-white block leading-tight">
                            {m.opponent_name || t.matches_waiting}
                          </span>
                      ...
                      <span className="text-xs font-bold text-zinc-500">{t.room_opponent}</span>
                    </div>
                  </div>
                  ...
                  <div className="p-3 rounded-2xl bg-zinc-950 border border-zinc-800/80 grid grid-cols-2 gap-2 text-center my-2">
                    <div>
                      <span className="text-[10px] text-zinc-500 uppercase font-semibold block">{t.stake}</span>
                      <span className="text-sm font-bold text-white font-mono">{m.stake} DH</span>
                    </div>
                    <div>
                      <span className="text-[10px] text-zinc-500 uppercase font-semibold block">{t.prize}</span>
                  ...
                  {m.status === 'OPEN' ? (
                    isCreator ? (
                      ...
                      >
                        ...
                        <span>{t.matches_join_btn} ({m.stake} DH)</span>
                      </button>
                    )
                  ) : (
                    <Link
