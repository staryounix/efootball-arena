...
export default function ProfilePage() {
  ...
  const handleWithdrawSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(num) || num <= 0) {
      setWithdrawMsg({ type: 'error', text: 'يرجى إدخال مبلغ صحيح' });
      return;
    }

    if (!user || user.balance < num) {
      setWithdrawMsg({ type: 'error', text: 'رصيدك غير كافٍ لهذا المبلغ' });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        setWithdrawMsg({ type: 'success', text: 'تم تسجيل طلب السحب بنجاح! سيتم تحويل المبلغ قريباً.' });
        setWithdrawAmount('');
        setDestinationInfo('');
        await refreshUser();
      } else {
      ...
      setWithdrawMsg({ type: 'error', text: 'مشكل في الاتصال بالخادم' });
    } finally {
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div className="md:col-span-7 rounded-3xl bg-zinc-900/80 border border-zinc-800 p-6 flex flex-col justify-between space-y-6">
          ...
          <div className="grid grid-cols-3 gap-3 p-4 rounded-2xl bg-zinc-950 border border-zinc-800/80 text-center">
            <div>
              <span className="text-[10px] text-zinc-500 uppercase font-semibold block">{t.lead_efootball_id}</span>
            ...
            <div>
              <span className="text-[10px] text-zinc-500 uppercase font-semibold block">{t.lead_wins} / {t.lead_losses}</span>
        ...
        {user.role === 'ADMIN' ? (
          <div className="md:col-span-5 rounded-3xl bg-gradient-to-br from-purple-950/40 via-zinc-900 to-zinc-950 border border-purple-500/40 p-6 flex flex-col justify-between shadow-2xl">
            ...
            <div className="my-4">
              <span className="text-xs text-zinc-400 block mb-1">الصلاحية:</span>
              <span className="text-2xl font-black text-white">{t.prof_admin_panel}</span>
          ...
          <div className="md:col-span-5 rounded-3xl bg-gradient-to-br from-emerald-950/40 via-zinc-900 to-zinc-950 border border-emerald-500/40 p-6 flex flex-col justify-between shadow-2xl">
            ...
            <div className="my-4">
              <span className="text-xs text-zinc-400 block mb-1">{t.prof_available}</span>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              ...
              >
                <ArrowUpRight className="w-4 h-4 text-amber-400 rtl:rotate-90" />
                <span>{t.prof_withdraw}</span>
              </button>
            </div>
          </div>
        )}
          ...
          <div className="relative w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-zinc-800">
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                <ArrowUpRight className="w-4 h-4 text-amber-400" /> {t.prof_withdraw_title}
              </h3>
            ...
            <form onSubmit={handleWithdrawSubmit} className="space-y-3 text-xs">
              <div>
                <label className="block text-zinc-300 font-semibold mb-1">{t.prof_withdraw_amount} (Max: {user.balance} DH)</label>
                <input
                  type="number"
                  min="20"
                  max={user.balance}
              ...
              <div>
                <label className="block text-zinc-300 font-semibold mb-1">{t.prof_withdraw_method}</label>
                <select
              ...
              >
                {withdrawLoading ? 'جاري الإرسال...' : t.prof_withdraw_submit}
              </button>
            </form>
          </div>
        </div>
          ...
          <div className="overflow-x-auto rounded-2xl border border-zinc-800 bg-zinc-900/60">
            <table className="w-full text-start text-xs">
              <thead className="bg-zinc-950 text-zinc-400 uppercase font-semibold border-b border-zinc-800">
                <tr>
                  <th className="p-3.5">{t.prof_tx_type}</th>
                  <th className="p-3.5">{t.prof_tx_desc}</th>
                  <th className="p-3.5">{t.prof_tx_date}</th>
                  <th className="p-3.5 text-end">{t.prof_tx_amount}</th>
                  <th className="p-3.5 text-end">{t.prof_tx_balance}</th>
                </tr>
              </thead>
                ...
                {transactions.map(item => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-zinc-800/40">
                      ...
                      <td className="p-3.5 text-zinc-300 font-medium">{item.description}</td>
                      <td className="p-3.5 text-zinc-500">{new Date(item.created_at).toLocaleString('fr-FR')}</td>
                      <td className={`p-3.5 text-end font-black font-mono ${isPositive ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {isPositive ? `+${item.amount}` : item.amount} DH
                      </td>
                      <td className="p-3.5 text-end font-mono text-zinc-400">
                        {item.balance_after.toFixed(2)} DH
                      </td>
                    </tr>
                  );
                })}
